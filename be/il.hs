...
data ReferentTy = RefScalarTy ScalarTy
               | StructTy ReferentTy --array
               | UnionTy ReferentTy --array
               | ParamTy TyParamIdx
               | OpaqueTy                
               | CodeTy              
               | NilTy                
...
let emit_full
  ...
  let emit_quad (q':quad') : unit =
    ...
    match q' with
        Unary { unary_dst = Mem (dst_mem, RefScalarTy src_st);
                unary_src = Cell (Mem (src_mem, RefScalarTy dst_st));
                unary_op = op }
          when is_mov op ->
            let v = next_vreg_cell e dst_st in
              emit_quad_bottom
                (unary op v (Cell (Mem (src_mem, RefScalarTy src_st))));
              emit_quad_bottom
                (unary op (Mem (dst_mem, RefScalarTy dst_st)) (Cell v))
