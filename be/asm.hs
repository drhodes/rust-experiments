...
checkdef kind name v inj =
    case v of
      Nothing -> Nothing --raise (Undef_sym (kind ^ " fixup " ^ name ^ " undefined in eval32"))
      Just x -> inj kind name x

eval :: Expr a b -> Integer
eval e = case e of
           IMM i -> i
           ...
           REM a b -> (eval a) `rem` (eval b)
           MAX a b -> (eval a) `max` (eval b)

           --ALIGN a, b) -> i32_align eval a) eval b)
           SLL a b -> shiftL (eval a) b
           SLR a b -> shift (eval a) b
           SAR a b -> shiftR (eval a) b
           AND a b -> (eval a) .&. (eval b)
           XOR a b -> (eval a) `xor` (eval b)
           OR a b ->  (eval a) .|. (eval b)
           NOT a -> complement (eval a)
           NEG a -> negate (eval a)

           F_POS f -> checkdef "file position" (fixup_name f) (fixup_file_pos f) expandInt                        
           F_SZ f -> checkdef "file size" (fixup_name f) (fixup_file_sz f) expandInt
{-
           M_POS f -> checkdef "mem position" (fixup_name f) (fixup_mem_pos f) chop64
           M_SZ f -> checkdef "mem size" (fixup_name f) (fixup_mem_sz f) chop64
           EXT i -> of_int i
